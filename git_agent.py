...
for tool in toolkit.get_tools():
    # if tool.name in {"Create Pull Request", "Set Active Branch"}:
      tool.name = tool.mode
      tools.append(tool)
...
5. Set the current active branch to the new branch
6. Create a pull request with appropriate title and description
7. Output a link so that I can create or view the pull request

Be concise and handle errors gracefully. Use the available GitHub and command-line tools effectively. PR Link: """),
    ("human", "{input}"),
...
class GitDocumentationAgent:
    ...
    def commit_and_create_pr(self, doc_files_pattern="**/*.md", commit_message="Auto-generated documentation update"):
        ...
        branch_name = f"docs/vibebase-docs-{timestamp}"
        
        # Find documentation files
        ptrs = gitignore.load_patterns('.')
        doc_files = glob.glob(doc_files_pattern, recursive=True)
        # doc_files = [
        #   f
        #   for f in doc_files
        #   if not gitignore.is_ignored(toolkit.get_tools(), ptrs)
        # ]
        ...
        2. Add and commit these documentation files: use git status, and only commit *.md files
        3. Use commit message: "{commit_message}"
        4. Push the branch '{branch_name}' to origin, remember to set upstream
        ...
        1. Create a new local branch named '{branch_name}' using shell
        2. use git status, and find all uncommited *.md files
        3. Push while setting upstream with 
        4. Create a new PR with these files and '{branch_name}' as the branch name, if it failed, retry one more time to create the PR
...
def main():
    ...
    git_agent = GitDocumentationAgent("farbodbj", "vibe-coders")
